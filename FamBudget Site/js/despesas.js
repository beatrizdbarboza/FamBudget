...
const CORES_CATEGORIA_API_DESPESAS = {
  ...
  category_brown: "#4B2B0A",
  category_orange: "#FF7700",
  category_pink: "#DA4175",
  category_purple: "#7B55E3",
  category_gray: "#4F595B",
  category_blue: "#00CCFF",
  category_green: "#44B948",
  category_yellow: "#E1CC0A"
};
...
const CATEGORIAS_FIXAS_DESPESAS = [
  {
    nome: "Alimentação",
    cor: "#00CCFF",
    corClasse: "categoryBlue",
    aliases: ["alimentacao", "alimentação"]
  },
  {
    nome: "Assinaturas",
    cor: "#FF7700",
    corClasse: "categoryOrange",
    aliases: [
      "assinaturas",
      "assinatura",
      "assin",
  ...
  {
    nome: "Contas pessoais",
    cor: "#DA4175",
    corClasse: "categoryPink",
    aliases: [
      "contas pessoais",
      "conta pessoal",
      "contas pessoaiss",
  ...
  },
  {
    nome: "Educação",
    cor: "#E1CC0A",
    corClasse: "categoryYellow",
    aliases: ["educacao", "educação"]
  },
  {
    nome: "Lazer",
    cor: "#7B55E3",
    corClasse: "categoryPurple",
    aliases: ["lazer"]
  },
  {
    nome: "Saúde",
    cor: "#44B948",
    corClasse: "categoryGreen",
    aliases: ["saude", "saúde"]
  },
  {
    nome: "Outros",
    cor: "#4F595B",
    corClasse: "categoryGray",
    aliases: ["outros", "outro"]
  }
];

const CORES_PADRAO_CATEGORIAS_DESPESAS = [
  "#00CCFF", 
  "#4B2B0A", 
  "#E1CC0A", 
  "#7B55E3", 
...
let dadosPagamentoDespesa;
let inputDataPagamentoDespesa;
let selectFormaPagamentoDespesa;
...
function pegarElementos() {
  ...
  dadosPagamentoDespesa = document.getElementById("dadosPagamentoDespesa");
  inputDataPagamentoDespesa = document.getElementById("dataPagamentoDespesa");
  selectFormaPagamentoDespesa = document.getElementById("formaPagamentoDespesa");
...
function configurarEventos() {
  ...
  if (checkPago) {
    checkPago.addEventListener("change", atualizarCamposPagamentoDespesa);
  }
...
function atualizarCamposPagamentoDespesa() {
  if (!dadosPagamentoDespesa) return;

  const pago = Boolean(checkPago?.checked);

  dadosPagamentoDespesa.style.display = pago ? "grid" : "none";

  if (pago) {
    if (inputDataPagamentoDespesa && !inputDataPagamentoDespesa.value) {
      inputDataPagamentoDespesa.value =
        inputData?.value || normalizarDataISO(new Date());
    }
  } else {
    if (inputDataPagamentoDespesa) inputDataPagamentoDespesa.value = "";
    if (selectFormaPagamentoDespesa) selectFormaPagamentoDespesa.value = "";
  }
}
...
function transformarEmArray(resposta) {
  ...
  if (listaDireta) return listaDireta;
  ...
  const pareceDespesa =
    ...
    resposta.value ||
    resposta.dateInitial ||
    resposta.categoryId ||
    resposta.typeExpense;
...
async function carregarCategoriasDespesaAPI() {
  ...
  categoriasDespesaAPI = aplicarCoresDiferentesCategoriasDespesa(
    removerCategoriasDuplicadasDespesa(
      lista
...
function normalizarCategoriaDespesaAPI(item) {
  ...
  return {
    ...
    cor: obterCorCategoriaDespesa(nome, "", ""),
    corClasse
  };
}
...
function aplicarCoresDiferentesCategoriasDespesa(categorias) {
  return categorias.map((categoria, index) => {
    const nomeNormalizado = normalizarTexto(categoria.nome);

    const corPorNome = CORES_CATEGORIA_POR_NOME_DESPESAS[nomeNormalizado];
    ...
    return {
      ...categoria,
      cor: corPorNome || coresReserva[index % coresReserva.length]
    };
  });
}

function montarCategoriasFixasDespesas(listaAPI) {
  const categoriasNormalizadas = removerCategoriasDuplicadasDespesa(
    listaAPI
      .map(normalizarCategoriaDespesaAPI)
      .filter((categoria) => {
        const id = Number(categoria.id);
  ...
  CATEGORIAS_FIXAS_DESPESAS.forEach((categoriaFixa) => {
    const encontrada = categoriasNormalizadas.find((categoriaAPI) => {
      const nomeAPI = normalizarTexto(categoriaAPI.nome);
      const corClasseAPI = normalizarTexto(categoriaAPI.corClasse);
      const corAPI = String(categoriaAPI.cor || "").toUpperCase();
      const corFixa = String(categoriaFixa.cor || "").toUpperCase();

      const batePorNome = categoriaFixa.aliases.some((alias) => {
        const aliasNormalizado = normalizarTexto(alias);

        return (
          nomeAPI === aliasNormalizado ||
          nomeAPI.includes(aliasNormalizado) ||
          aliasNormalizado.includes(nomeAPI)
        );
      });

      const batePorCorClasse =
        corClasseAPI &&
        normalizarTexto(categoriaFixa.corClasse) === corClasseAPI;
      ...
      return batePorNome || batePorCorClasse || batePorCorHex;
    });

    if (encontrada) {
      categoriasFinal.push({
        id: encontrada.id,
        nome: categoriaFixa.nome,
        cor: categoriaFixa.cor,
        corClasse: categoriaFixa.corClasse
      });
    } else {
      console.warn("Categoria fixa não encontrada na API:", categoriaFixa.nome);
    }
  });
...
function obterCorCategoriaDespesa(nome, corClasse = "", corHex = "") {
  ...
  if (CORES_CATEGORIA_POR_NOME_DESPESAS[texto]) {
    return CORES_CATEGORIA_POR_NOME_DESPESAS[texto];
  }

  if (corHex && /^#([0-9A-F]{3}){1,2}$/i.test(corHex)) {
    return corHex;
  }

  const classeOriginal = String(corClasse || "").trim();

  if (CORES_CATEGORIA_API_DESPESAS[classeOriginal]) {
    return CORES_CATEGORIA_API_DESPESAS[classeOriginal];
  }

  const classeNormalizada = classeOriginal
    .replace(/\s/g, "")
    .replace(/-/g, "_")
    .toLowerCase();

  if (CORES_CATEGORIA_API_DESPESAS[classeNormalizada]) {
    return CORES_CATEGORIA_API_DESPESAS[classeNormalizada];
  }
...
function getChavePagamentosDespesas() {
  return `${getUserKeyDespesa()}_pagamentosDespesas`;
}

function lerPagamentosDespesas() {
  try {
    const dados = localStorage.getItem(getChavePagamentosDespesas());
...
function salvarPagamentosDespesas(lista) {
  localStorage.setItem(
    getChavePagamentosDespesas(),
    JSON.stringify(Array.isArray(lista) ? lista : [])
  );
}

function salvarPagamentoLocalDespesa(expenseId, payload) {
  if (!expenseId || !payload) return;

  const listaAtual = lerPagamentosDespesas();

  const pagamento = {
    expenseId: String(expenseId),
    datePayment: payload.datePayment,
    typePaymentId: Number(payload.typePaymentId || 0)
  };

  const novaLista = listaAtual.filter((item) => {
    return String(item.expenseId) !== String(expenseId);
  });

  novaLista.push(pagamento);
  salvarPagamentosDespesas(novaLista);
}

function buscarPagamentoLocalDespesa(expenseId) {
  if (!expenseId) return null;

  const lista = lerPagamentosDespesas();

  return (
    lista.find((item) => String(item.expenseId) === String(expenseId)) ||
    null
  );
}

function removerPagamentoLocalDespesa(expenseId) {
  if (!expenseId) return;
...
async function carregarDespesasAPI() {
  ...
  if (usuarioPertenceFamiliaDespesa) {
    ...
    if (familyId) {
      ...
      if (respostaFamilia.ok) {
        listaAPI = [
          ...listaAPI,
          ...transformarEmArray(respostaFamilia.data)
        ];
      }
    }
    ...
    if (respostaUser.ok) {
      listaAPI = [
        ...listaAPI,
        ...transformarEmArray(respostaUser.data)
      ];
    }
  } else {
    membrosFamiliaDespesa = [];
  ...
  const fixasBase = await buscarDespesasFixasDosUltimos12Meses();

  listaAPI = [
    ...transformarEmArray(listaAPI),
    ...transformarEmArray(fixasBase)
  ];

  let listaNormalizada = transformarEmArray(listaAPI)
...
function despesaFixaAtivaNoMes(despesa) {
  const tipo = normalizarTipoDespesa(
    despesa?.typeExpenseId ??
    despesa?.type_expense_id ??
    despesa?.typeExpense ??
    despesa?.tipo ??
    despesa?.type ??
    "variavel"
  ...
  if (tipo !== "fixa") return false;

  const dataBaseISO = normalizarDataISO(
    despesa?.dateInitial ||
    despesa?.data ||
    despesa?.date ||
    ""
  ...
  if (!dataBaseISO) return false;

  const dataBase = converterData(dataBaseISO);

  if (!dataBase) return false;

  const diferencaMeses =
    (dataMesAtual.getFullYear() - dataBase.getFullYear()) * 12 +
    (dataMesAtual.getMonth() - dataBase.getMonth());

  return diferencaMeses >= 0 && diferencaMeses <= 11;
}

function obterDataExibicaoDespesa(despesa) {
  const tipo = normalizarTipoDespesa(
    despesa?.typeExpenseId ??
    despesa?.type_expense_id ??
    despesa?.typeExpense ??
  ...
  if (tipo !== "fixa") {
    return normalizarDataISO(dataOriginal);
  }

  if (!despesaFixaAtivaNoMes(despesa)) {
    return normalizarDataISO(dataOriginal);
  }

  const dataBase = converterData(dataOriginal);
  ...
  const ano = dataMesAtual.getFullYear();
  const mes = dataMesAtual.getMonth();
...
async function buscarDespesasFixasDosUltimos12Meses() {
  const chamadas = [];

  for (let i = 0; i < 12; i++) {
    const dataBusca = new Date(
      dataMesAtual.getFullYear(),
      dataMesAtual.getMonth() - i,
      1
    );
    ...
    chamadas.push(
      apiDespesas(`/expense/user/type/1?month=${mes}&year=${ano}`, {
        method: "GET",
        headers: headersDespesas(false)
      })
    );
  ...
  respostas.forEach((resposta) => {
    if (resposta?.ok) {
      fixas = [
        ...fixas,
        ...transformarEmArray(resposta.data)
      ];
    }
  });

  return removerDuplicadasDespesas(fixas.map(normalizarDespesaAPI));
}

function projetarDespesasFixasParaMesAtual(listaAtual, fixasBase) {
  const lista = transformarEmArray(listaAtual);
  const fixas = transformarEmArray(fixasBase);

  const projetadas = [];

  fixas.forEach((despesa) => {
    const tipo = normalizarTipoDespesa(
      despesa?.typeExpenseId ??
      despesa?.type_expense_id ??
      despesa?.typeExpense ??
    ...
    const diferencaMeses = calcularDiferencaMesesDespesa(
      dataBase,
      dataMesAtual
    );
    ...
    /*
      ...
      Julho = 2
      ...
      Até 11 = 12 meses contando o mês inicial.
    */
    if (diferencaMeses < 0 || diferencaMeses > 11) return;

    const dataProjetada = criarDataDaFixaNoMesAtual(dataBase);

    const jaExiste = lista.some((item) => {
      return despesasSaoEquivalentesNoMes(item, despesa, dataProjetada);
    });
    ...
    projetadas.push({
      ...
      id: `${despesa.id || despesa.expenseId || despesa.expense_id}-fixa-${pegarAnoMes(dataProjetada)}`,
      idOriginalFixa: despesa.id || despesa.expenseId || despesa.expense_id,

      data: dataProjetada,
      date: dataProjetada,
      dateInitial: dataProjetada,
...
function calcularDiferencaMesesDespesa(dataInicial, dataReferencia) {
  const data = new Date(`${normalizarDataISO(dataInicial)}T00:00:00`);

  if (isNaN(data.getTime())) return -1;

  return (
    (dataReferencia.getFullYear() - data.getFullYear()) * 12 +
    (dataReferencia.getMonth() - data.getMonth())
  );
}

function criarDataDaFixaNoMesAtual(dataInicial) {
  const data = new Date(`${normalizarDataISO(dataInicial)}T00:00:00`);

  if (isNaN(data.getTime())) {
    return normalizarDataISO(dataMesAtual);
  }
...
function despesasSaoEquivalentesNoMes(itemAtual, fixaBase, dataProjetada) {
  const descricaoAtual = normalizarTexto(
    itemAtual?.descricao ||
  ...
  const descricaoFixa = normalizarTexto(
    fixaBase?.descricao ||
    fixaBase?.description ||
    fixaBase?.name ||
    ""
  );
  ...
  const categoriaAtual = Number(
    itemAtual?.categoriaId ??
    itemAtual?.categoryId ??
    itemAtual?.category_id ??
    0
  );

  const categoriaFixa = Number(
    fixaBase?.categoriaId ??
    fixaBase?.categoryId ??
    fixaBase?.category_id ??
    0
  );

  const mesAtual = pegarAnoMes(
    itemAtual?.data ||
    itemAtual?.dateInitial ||
    itemAtual?.date ||
    ""
  );

  return (
    descricaoAtual === descricaoFixa &&
    Number(valorAtual) === Number(valorFixa) &&
    categoriaAtual === categoriaFixa &&
    mesAtual === pegarAnoMes(dataProjetada)
  );
}
...
const tipo = normalizarTipoDespesa(
  ...
  const pagamentoLocal = buscarPagamentoLocalDespesa(idNormalizado);

  const pagamentoAPI =
    despesa?.payment ||
    despesa?.pagamento ||
    despesa?.paymentExpense ||
    despesa?.expensePayment ||
    null;

  const pagamentoFinal = pagamentoAPI || pagamentoLocal;

  const pagoAPI = Boolean(
    despesa?.pago === true ||
    despesa?.paid === true ||
    despesa?.isPaid === true ||
    despesa?.status === "pago" ||
  ...
  const pagoFinal = Boolean(pagoAPI || pagamentoFinal);
  ...
  return {
    ...
    tipo,
    typeExpense: despesa?.typeExpense || despesa?.typeExpenseId || obterTipoExpenseNumericoDespesa(tipo),
    ...
    datePayment:
      pagamentoFinal?.datePayment ||
      pagamentoFinal?.paymentDate ||
      pagamentoFinal?.paidAt ||
      despesa?.datePayment ||
      despesa?.paymentDate ||
      despesa?.paidAt ||
      "",
    ...
    typePaymentId:
      pagamentoFinal?.typePaymentId ||
      pagamentoFinal?.type_payment_id ||
      pagamentoFinal?.typePayment?.id ||
      despesa?.typePaymentId ||
      despesa?.type_payment_id ||
      null,

    pago: pagoFinal,
    paid: pagoFinal,
    status: pagoFinal ? "pago" : "pendente",
...
function normalizarTipoDespesa(tipo) {
  if (tipo === null || tipo === undefined) return "variavel";

  if (typeof tipo === "object") {
    const idTipo =
      tipo.id ||
      tipo.typeExpenseId ||
      tipo.type_expense_id ||
      tipo.value ||
      null;
    ...
    if (idTipo) {
      return normalizarTipoDespesa(idTipo);
    }

    if (nomeTipo) {
      return normalizarTipoDespesa(nomeTipo);
    }
  ...
  if (
    ...
    texto === "sazonal" ||
    texto === "seasonal" ||
    texto === "seasonal_expense" ||
    texto.includes("sazon")
  ) {
    return "sazonal";
  }
...
function aplicarFiltros() {
  ...
  despesasFiltradas = despesas.filter((despesa) => {
    const dataExibicao = obterDataExibicaoDespesa(despesa);
    const mesmoMes = pegarAnoMes(dataExibicao) === anoMesAtual;
...
function renderizarTabelaDespesas() {
  ...
  if (!despesasFiltradas || !despesasFiltradas.length) {
  ...
  pagina.forEach((despesa) => {
    ...
    const statusHTML =
      despesa.status === "pago"
        ? `
          <div class="status-pagamento-cell">
            <strong>Pago</strong>
            <small>${formatarDataBR(pegarDataPagamentoDespesa(despesa))}</small>
            <small>${escapeHTML(pegarFormaPagamentoDespesa(despesa))}</small>
          </div>
        `
        : "Pendente";

    const usuarioHTML = mostrarUsuarioDespesa
      ? `<td class="usuario-despesa">${escapeHTML(pegarNomeAutorDespesa(despesa))}</td>`
      : "";

    tr.innerHTML = `
      <td>${formatarDataBR(obterDataExibicaoDespesa(despesa))}</td>

      <td>
        <div class="descricao-cell">
          <span class="icone-item icone-despesa">↓</span>
          <span>${escapeHTML(limparDescricaoParcela(despesa.descricao || despesa.description || despesa.name || "Sem descrição"))}</span>
        </div>
      </td>

      <td>
        <div class="categoria-cell">
          <span class="bolinha-categoria" style="background-color: ${despesa.corCategoria || "#4F595B"};"></span>
          <span>${escapeHTML(despesa.categoria || despesa.categoryName || "Outros")}</span>
        </div>
      </td>
      ...
      <td>
        <div class="tipo-cell">
          ...
          <span>${formatarTipoDespesa(despesa.tipo || "variavel")}</span>
        </div>
      </td>

      <td>${statusHTML}</td>

      <td class="valor negativo">${formatarMoedaPositiva(despesa.valor || despesa.value || despesa.amount || 0)}</td>
    ...
    const btnEditar = tr.querySelector(".btn-editar-despesa");
    const btnExcluir = tr.querySelector(".btn-excluir-despesa");

    if (btnEditar) {
      btnEditar.addEventListener("click", () => confirmarEditarDespesa(despesa.id));
    }

    if (btnExcluir) {
      btnExcluir.addEventListener("click", () => confirmarExcluirDespesa(despesa.id));
    }
...
function pegarPagamentoDespesa(despesa) {
  return (
    despesa?.payment ||
    despesa?.pagamento ||
    despesa?.paymentExpense ||
...
function pegarDataPagamentoDespesa(despesa) {
  const pagamento = pegarPagamentoDespesa(despesa);
  const pagamentoLocal = buscarPagamentoLocalDespesa(
    despesa?.expenseId || despesa?.expense_id || despesa?.id
  );

  return normalizarDataISO(
    pagamento?.datePayment ||
    pagamento?.paymentDate ||
    pagamento?.paidAt ||
    pagamento?.date ||
    pagamentoLocal?.datePayment ||
    despesa?.datePayment ||
    despesa?.paymentDate ||
    despesa?.paidAt ||
    ""
...
function pegarFormaPagamentoDespesa(despesa) {
  const pagamento = pegarPagamentoDespesa(despesa);
  const pagamentoLocal = buscarPagamentoLocalDespesa(
    despesa?.expenseId || despesa?.expense_id || despesa?.id
  );
  ...
  const forma =
    pagamento?.typePaymentId ||
    pagamento?.type_payment_id ||
    pagamento?.typePayment?.id ||
    pagamento?.type_payment?.id ||
    pagamentoLocal?.typePaymentId ||
    despesa?.typePaymentId ||
    despesa?.type_payment_id ||
    "";

  return formatarFormaPagamentoDespesa(forma);
}

function formatarFormaPagamentoDespesa(forma) {
  const id = Number(forma);
  ...
  return mapa[id] || "Forma não informada";
}
...
function limparCampos() {
  ...
  if (inputDataPagamentoDespesa) inputDataPagamentoDespesa.value = "";
  if (selectFormaPagamentoDespesa) selectFormaPagamentoDespesa.value = "";
  atualizarCamposPagamentoDespesa();
...
async function salvarDespesa() {
  ...
  try {
    ...
    if (pago) {
      const dataPagamento = inputDataPagamentoDespesa?.value || "";
      const typePaymentId = Number(selectFormaPagamentoDespesa?.value || 0);

      if (!dataPagamento || !typePaymentId) {
        mostrarMensagemDespesa("Informe a data do pagamento e a forma de pagamento.");
        return;
      }
    }
    ...
    if (typeExpense === 1 || normalizarTexto(tipoSelecionado).includes("fix")) {
      for (let i = 0; i < 12; i++) {
        payloads.push(
    ...
    for (const payload of payloads) {
      ...
      const despesaCriada = resposta.data?.data || resposta.data;

      const expenseId =
        despesaCriada?.id ||
        despesaCriada?.expenseId ||
        despesaCriada?.expense_id ||
        payload.id ||
        null;

      if (pago && expenseId) {
        const payloadPagamento = montarPayloadPagamentoDespesa();

        const respostaPagamento = await pagarDespesaAPI(
          expenseId,
          payloadPagamento
        );
        ...
        if (!respostaPagamento.ok) {
          mostrarMensagemDespesa(
            pegarMensagemErroDespesa(
              respostaPagamento.data,
              "A despesa foi criada, mas não foi possível registrar o pagamento."
            )
          );
          return;
        }

        salvarPagamentoLocalDespesa(expenseId, payloadPagamento);
      }
    }
    ...
    mostrarMensagemDespesa(
      pago
        ? "Despesa salva e pagamento registrado com sucesso!"
...
async function atualizarDespesaEditada() {
  ...
  if (!idApi) {
    mostrarMensagemDespesa("ID da despesa não encontrado para atualizar.");
    return;
  }
  ...
  const payload = await montarPayloadAtualizarDespesa({
    descricao,
    valor: valorTotal,
    data,
    categoriaId,
  ...
  if (!respostaPut.ok) {
    mostrarMensagemDespesa(
      pegarMensagemErroDespesa(
        respostaPut.data,
        "Não foi possível atualizar a despesa na API."
      )
    );
    return;
  }
  ...
  if (pago) {
    ...
    let respostaPagamento = await atualizarPagamentoDespesaAPI(
      idApi,
      payloadPagamento
    );

    if (!respostaPagamento.ok) {
      respostaPagamento = await pagarDespesaAPI(
        idApi,
        payloadPagamento
      );
    }
    ...
    if (!respostaPagamento.ok) {
      mostrarMensagemDespesa(
        pegarMensagemErroDespesa(
          respostaPagamento.data,
          "Despesa atualizada, mas não foi possível registrar o pagamento."
        )
      );
      return;
    }

    salvarPagamentoLocalDespesa(idApi, payloadPagamento);
  } else {
    removerPagamentoLocalDespesa(idApi);
  }

  removerDespesaEditadaLocalmente(despesaOriginal);
...
async function montarPayloadAtualizarDespesa({
  descricao,
  valor,
  data,
  categoriaId,
  ...
  if (usuarioPertenceFamiliaDespesa) {
    familyId = await obterFamilyIdDespesa();
  }
...
async function atualizarDespesaAPI(expenseId, payload) {
  return apiDespesas(`/expense/${expenseId}`, {
    method: "PUT",
    headers: headersDespesas(true),
    body: JSON.stringify(payload)
  });
...
async function pagarDespesaAPI(expenseId, payload) {
  return apiDespesas(`/payment/expense/${expenseId}`, {
    method: "POST",
    headers: headersDespesas(true),
    body: JSON.stringify(payload)
  });
...
function montarPayloadPagamentoDespesa() {
  const dataPagamento =
    inputDataPagamentoDespesa?.value ||
    inputData?.value ||
    normalizarDataISO(new Date());
...
async function atualizarPagamentoDespesaAPI(expenseId, payload) {
  return apiDespesas(`/payment/expense/${expenseId}`, {
    method: "PUT",
    headers: headersDespesas(true),
    body: JSON.stringify(payload)
...
async function removerPagamentoDespesaAPI(expenseId) {
  return apiDespesas(`/payment/expense/${expenseId}`, {
    method: "DELETE",
    headers: headersDespesas(false)
  });
...
function preencherModalEdicaoDespesa(id) {
  ...
  if (checkPago) {
    checkPago.checked = Boolean(despesa.pago || despesa.paid || despesa.status === "pago");
  }
  ...
  if (inputDataPagamentoDespesa) {
    inputDataPagamentoDespesa.value = pegarDataPagamentoDespesa(despesa) || "";
  }

  if (selectFormaPagamentoDespesa) {
    const pagamento = pegarPagamentoDespesa(despesa);
    ...
    const forma =
      ...
      despesa?.methodPayment ||
      "";

    selectFormaPagamentoDespesa.value = forma || "";
  }
...
function despesaFoiExcluida(despesa) {
  ...
  return excluidas.some((item) => {
    ...
    const mesmoId =
      idsDespesa.length > 0 &&
      idsItem.length > 0 &&
      idsDespesa.some((id) => idsItem.includes(id));
    ...
    return (
      descricaoItem === descricaoDespesa &&
      categoriaItem === categoriaDespesa &&
      Number(valorItem) === Number(valorDespesa) &&
      dataItem === dataDespesa
    );
  });
}
