...
function obterMoedaUsuarioOrcamento() {
  const email =
    sessionStorage.getItem("emailUsuario") ||
  ...
  console.log("MOEDA USADA EM ORÇAMENTOS:", moeda);
...
function formatarMoedaOrcamento(valor) {
  return Number(valor || 0).toLocaleString("pt-BR", {
    style: "currency",
    currency: obterMoedaUsuarioOrcamento()
  });
}

/*
  Mantido porque seu renderizar() usa esse nome.
*/
function formatarMoedaReceita(valor) {
  return formatarMoedaOrcamento(valor);
}

/*
  Mantido caso alguma outra parte use o nome no plural.
*/
...
function carregarTransacoes() {
  ...
  */
  ...
  const transacoesUsuario = lerStorageUsuario("transacoes", null);
...
function carregarFiltroCategorias() {
  ...
  orcamentos.forEach((orcamento) => {
    if (!categoriasFixas.includes(orcamento.categoria)) {
      const jaExiste = [...filtroCategoria.options].some((option) => {
        return option.value === orcamento.categoria;
      });
