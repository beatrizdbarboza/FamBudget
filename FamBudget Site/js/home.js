console.log("HOME.JS OK - REFEITO SEM ALTERAR ESTILO");
...
async function apiGetHome(path, mostrarErro = false) {
  ...
  try {
    ...
    console.log("HOME TESTE ENDPOINT:", {
      path,
      status: res.status,
      ok: res.ok,
      data
...
/* ================= FILTRO DO USUÁRIO QUANDO NÃO TEM FAMÍLIA ================= */
...
function itemEstaExcluidoHome(item, tipo) {
  ...
  return excluidos.some((excluido) => {
    ...
    /*
      Compatibilidade com a chave que o receitas.js salva.
      Mesmo que a categoria venha diferente, compara descrição + valor + data.
    */
    return (
      descricaoItem &&
...
async function carregarDadosFinanceiros() {
  ...
  try {
    ...
    console.log("HOME - USUÁRIO TEM FAMÍLIA:", usuarioPertenceFamiliaHome);
    console.log("HOME - FAMILY ID:", familyId);
    ...
    if (usuarioPertenceFamiliaHome) {
      /*
        IMPORTANTE:
        Quando está em família, NÃO pode cair em /income/user nem /expense/user,
        porque esses endpoints mostram somente os dados da Bianca.
      */
    ...
    /*
      Sem família: filtra pelo usuário logado.
      Com família: NÃO filtra, porque precisa mostrar dados de todos.
    */
    if (!usuarioPertenceFamiliaHome) {
    ...
    console.log("HOME - RECEITAS FINAIS:", receitas);
    console.log("HOME - DESPESAS FINAIS:", despesas);
...
function calcularResumoMesAtualHome(receitas, despesas) {
  ...
  /*
    IMPORTANTE:
    Os endpoints já estão buscando pelo mês e ano:
    /income/family/{id}?month=5&year=2026
    /expense/family/{id}?month=5&year=2026

    Então NÃO precisa filtrar novamente pela data aqui.
    Se filtrar de novo e a API mandar a data em outro campo/formato,
    a Home recebe os dados, mas soma 0.
  */
  ...
  console.log("HOME - TOTAL RECEITAS CALCULADO:", totalReceitas);
  console.log("HOME - TOTAL DESPESAS CALCULADO:", totalDespesas);
