console.log("DADOS.JS OK - PDF CORRIGIDO COM FAMÍLIA, USUÁRIO, CATEGORIA E CORES");

const API_DADOS_URL = "https://www.manage-control-dev.com.br/api/v1";

/* =========================
   CORES DAS CATEGORIAS NO PDF
========================= */
...
async function carregarCategoriasDados() {
  ...
  console.log("PDF - CATEGORIAS API:", categorias);
...
async function buscarDadosFinanceirosParaPDF() {
  ...
  if (familia.emFamilia && familia.familyId) {
    ...
    console.log("PDF - RECEITAS FAMÍLIA:", receitasResposta);
    console.log("PDF - DESPESAS FAMÍLIA:", despesasResposta);
  } else {
    ...
    console.log("PDF - RECEITAS USUÁRIO:", receitasResposta);
    console.log("PDF - DESPESAS USUÁRIO:", despesasResposta);
  }
...
function normalizarItemPDF(item, tipoPadrao, categorias) {
  ...
  console.log("PDF - ITEM NORMALIZADO:", {
    tipo,
    descricao: item?.description || item?.descricao || item?.name,
