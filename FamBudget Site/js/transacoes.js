console.log("TRANSACOES.JS OK - API + FAMÍLIA + EXCLUÍDOS + EDIÇÕES LOCAIS");
...
async function carregarCategoriasExtratos() {
  ...
  console.log("TRANSACOES - CATEGORIAS API:", categoriasExtratos);
}
...
async function carregarExtratos() {
  ...
  let receitasResposta = null;
  let despesasResposta = null;

  if (usuarioPertenceFamiliaExtratos) {
    const familyId = await obterFamilyIdExtratos();

    console.log("TRANSACOES - USUÁRIO TEM FAMÍLIA:", usuarioPertenceFamiliaExtratos);
    console.log("TRANSACOES - FAMILY ID:", familyId);
    ...
    if (familyId) {
      ...
      const receitasFamiliaLista = transformarEmArrayExtratos(receitasFamilia);
      const despesasFamiliaLista = transformarEmArrayExtratos(despesasFamilia);

      console.log("TRANSACOES - RECEITAS FAMÍLIA:", receitasFamiliaLista);
      console.log("TRANSACOES - DESPESAS FAMÍLIA:", despesasFamiliaLista);

      receitasResposta = receitasFamiliaLista;
      despesasResposta = despesasFamiliaLista;
    }
  } else {
    receitasResposta = await apiGetExtratos(`/income/user?month=${mes}&year=${ano}`);
    despesasResposta = await apiGetExtratos(`/expense/user?month=${mes}&year=${ano}`);
  }

  let receitasApi = transformarEmArrayExtratos(receitasResposta)
    .map((item) => {
      return normalizarTransacaoExtratos(
        {
          ...item,
  ...
  let despesasApi = transformarEmArrayExtratos(despesasResposta)
    .map((item) => {
      return normalizarTransacaoExtratos(
        {
          ...item,
  ...
  transacoesApi = aplicarEdicoesLocaisExtratos(transacoesApi);
  ...
  /*
    Sem família: mostra só dados do usuário logado.
    Com família: mostra dados de todos os membros.
  */
  if (!usuarioPertenceFamiliaExtratos) {
    transacoesApi = transacoesApi.filter((item) => {
  ...
  console.log("EXTRATOS API USADOS:", ultimasTransacoes);
...
function despesaFoiExcluidaExtratos(item) {
  ...
  const categoriaItem = normalizarTextoExtratos(
    item.category ||
    item.categoria ||
    item.categoryName ||
    item.nomeCategoria ||
  ...
  return excluidas.some((excluida) => {
    ...
    */
    ...
    /*
      Segunda comparação: descrição + valor + data.
      Categoria não é obrigatória aqui porque às vezes a API volta "Outros"
      e a tela editada está como "Alimentação".
    */
    const descricaoExcluida = normalizarTextoExtratos(
    ...
    const dataExcluida = formatarDataISOExtratos(
    ...
    if (mesmaDescricao && mesmoValor && mesmaData) {
      return true;
    }

    /*
      Terceira comparação: se for uma despesa editada localmente e depois excluída.
    */
    const valorOriginalExcluido = Number(
      excluida.valueOriginal ??
      excluida.valorOriginal ??
      excluida.amountOriginal ??
      0
    );

    const dataOriginalExcluida = formatarDataISOExtratos(
      excluida.dateInitialOriginal ||
      excluida.dataOriginal ||
      excluida.dateOriginal ||
      ""
    );

    const mesmoValorOriginal =
      valorOriginalExcluido > 0 &&
      Number(valorItem || 0).toFixed(2) === Number(valorOriginalExcluido || 0).toFixed(2);

    const mesmaDataOriginal =
      dataItem &&
      dataOriginalExcluida &&
      dataItem === dataOriginalExcluida;

    return mesmaDescricao && mesmoValorOriginal && mesmaDataOriginal;
  });
}
