...
async function atualizarDespesaEditada() {
  ...
  if (!respostaPut.ok) {
    console.warn("PUT falhou:", respostaPut);
...
function montarPayloadDespesa({ descricao, valor, data, pago = false, categoriaId, tipo, typeExpense = null }) {
  ...
  const fixa = tipoNumerico === 1 || normalizarTexto(tipo).includes("fix");
  ...
  const payload = {
    ...
    recurring: Boolean(fixa),
    recorrente: Boolean(fixa),
    monthly: Boolean(fixa),
    repeatMonthly: Boolean(fixa)
  };
