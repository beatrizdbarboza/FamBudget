...
const STORAGE_RECEITAS_LOCAIS = "receitasLocais";
...
function normalizarTipoReceita(tipo) {
  ...
  if (texto === "1" || texto.includes("fix")) {
    return "fixa";
  }

  if (texto === "3" || texto.includes("sazon")) {
    return "sazonal";
  }

  if (texto === "2" || texto.includes("vari")) {
    return "variavel";
  }
...
function formatarTipoReceita(tipo) {
  const tipoNormalizado = normalizarTipoReceita(tipo);

  if (tipoNormalizado === "fixa") return "Fixa";
  if (tipoNormalizado === "sazonal") return "Sazonal";

  return "Variável";
}

function extrairTipoReceitaAPI(receita) {
  ...
  if (tipoEncontrado) {
    return normalizarTipoReceita(tipoEncontrado);
  }
...
async function carregarReceitasAPI() {
  ...
  const receitasAPI = listaAPI.map((item, index) => {
    return normalizarReceitaAPI(item, index);
  });

  const receitasLocais = lerReceitasLocais().map((item, index) => {
    return normalizarReceitaAPI(item, `local-${index}`);
  });
...
async function salvarReceita() {
  ...
  try {
    ...
    const receitaLocal = criarReceitaLocalDoPayload(payload, resposta.data);

    salvarReceitaLocalmente(receitaLocal);

    receitas = removerDuplicadasReceitas([
      normalizarReceitaAPI(receitaLocal, Date.now()),
      ...receitas
    ]);
...
function montarPayloadReceita({ descricao, valor, data, tipo, mensal = false }) {
  const tipoNormalizado = normalizarTipoReceita(tipo);
  const ehFixa = tipoNormalizado === "fixa" || Boolean(mensal);
  ...
  return {
    ...
    incomeType: tipoNormalizado,

    salvarTodoMes: ehFixa,
    recorrente: ehFixa
  };
}
...
async function excluirReceita(id) {
  ...
  removerReceitaLocalmente(receita);
...
function getChaveReceitasLocais() {
  return getChaveUsuarioReceita(STORAGE_RECEITAS_LOCAIS);
}

function lerReceitasLocais() {
  return [
    ...lerListaStorage(STORAGE_RECEITAS_LOCAIS),
    ...lerListaStorage(getChaveReceitasLocais())
  ];
}

function salvarReceitasLocais(lista) {
  const listaFinal = Array.isArray(lista) ? lista : [];

  salvarListaStorage(STORAGE_RECEITAS_LOCAIS, listaFinal);
  salvarListaStorage(getChaveReceitasLocais(), listaFinal);
}

function criarReceitaLocalDoPayload(payload, respostaData = null) {
  const base =
    respostaData && typeof respostaData === "object"
      ? respostaData
      : {};
  ...
  const idGerado =
    ...
    base.incomeId ||
    base.income_id ||
    `receita-local-${Date.now()}`;

  return {
    ...base,

    id: idGerado,
    incomeId: base.incomeId || base.income_id || base.id || idGerado,
    income_id: base.income_id || base.incomeId || base.id || idGerado,
    ...
    incomeType: normalizarTipoReceita(
      base.incomeType ||
      base.tipo ||
      base.type ||
      payload.incomeType ||
    ...
    salvarTodoMes: Boolean(
      base.salvarTodoMes ||
      base.recorrente ||
      payload.salvarTodoMes ||
      payload.recorrente
    ),
    ...
    userId: getIdUsuarioAtualReceita(),
    autorEmail: getEmailUsuarioAtualReceita(),
    autorNickname: getNicknameUsuarioAtualReceita()
  };
}

function salvarReceitaLocalmente(receita) {
  if (!receita) return;

  const listaAtual = lerReceitasLocais();
  const normalizada = normalizarReceitaAPI(receita, Date.now());

  const novaLista = removerDuplicadasReceitas([
    normalizada,
    ...listaAtual
  ]);

  salvarReceitasLocais(novaLista);
}

function removerReceitaLocalmente(receita) {
  if (!receita) return;
  ...
  const novaLista = listaAtual.filter((item) => {
    ...
    return !idsReceita.some((id) => idsItem.includes(id));
  });
...
function normalizarReceitaAPI(receita, index = 0) {
  ...
  return {
    ...
    tipo: extrairTipoReceitaAPI(receita),
